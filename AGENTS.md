...
**Never ask for confirmation.** If there's an active plan, keep executing steps one by one until all are done. No questions, no pauses, no "should I continue?" — just do the work.
...
├── autoload/SpaceVim.vim          # SpaceVim core file (Lua delegate)
├── autoload/SpaceVim/api/         # Public APIs (Lua delegates)
├── autoload/SpaceVim/layers/      # available layers (Lua delegates)
├── autoload/SpaceVim/plugins/     # builtin plugins (Lua delegates)
├── autoload/SpaceVim/mapping/     # mapping guide (Lua delegates)
├── colors/                        # default colorscheme
├── docker/                        # docker image generator
├── bundle/                        # bundled plugins (git submodules / cloned)
├── lua/spacevim/                  # Lua core (primary implementation)
│   ├── init.lua                   # entry point (begin/end/welcome)
│   ├── default.lua                # default options & keybindings
│   ├── layer.lua                  # layer manager
│   ├── plugins.lua                # plugin manager
│   ├── logger.lua                 # logger
│   ├── command.lua                # command definitions
│   ├── autocmds.lua               # autocmd group
│   ├── server.lua                 # server manager
│   ├── health.lua                 # health checker
│   ├── lsp.lua                    # LSP client
│   ├── diagnostic.lua             # diagnostic module
│   ├── api/                       # API modules
...
- **Lua** is the primary implementation in `lua/spacevim/**` - targets Neovim 0.9+.
- **Vim script** files in `autoload/SpaceVim/**` are thin delegates that call `lua require('spacevim...')`.
- **Layers** go in `autoload/SpaceVim/layers/` (thin delegate) + `lua/spacevim/layers/` (logic).
- **APIs** go in `autoload/SpaceVim/api/` (thin delegate) + `lua/spacevim/api/` (logic).
